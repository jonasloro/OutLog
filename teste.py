...
# ==========================================
# FUNÇÕES DE CAPACIDADE E ESTOQUE
# ==========================================
...
# ==========================================
# FUNÇÕES DE GERAÇÃO DE CASULOS
# ==========================================
def gerar_chaves_casulo(estrutura):
    """Gera todas as chaves de casulos baseado na estrutura"""
    chaves = []
...
def gerar_estrutura_rua_para_visualizacao(estrutura):
    """Gera dados para visualização da estrutura"""
    dados = []
    ...
    for rua, cfg in estrutura.items():
        ...
        dados.append({
            ...
            "Colunas": total_cols,
            "Níveis": niveis,
            "Casulos": total_casulos,
            "Gênero": genero,
            "Metal": "✓" if metal else "",
            "Metal Cols": "✓" if metal_cols else "",
            "Lado Ímpar": len(cols_impar),
            "Lado Par": len(cols_par),
            "Seq": len(cols_seq)
        })
...
def mostrar_metricas_cds(estrutura):
    """Mostra métricas resumidas do CD"""
    total_casulos = 0
    total_ruas = 0
    total_colunas = 0
    ...
    with col1:
        ...
        <div class="metric-box">
            <div class="metric-value">{total_ruas}</div>
            <div class="metric-label">Ruas Ativas</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-box">
            <div class="metric-value">{total_casulos:,}</div>
            <div class="metric-label">Casulos Totais</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f"""
        <div class="metric-box">
            <div class="metric-value" style="font-size: 1.2em;">👩 {fem:,} | 👨 {masc:,}</div>
            <div class="metric-label">Feminino / Masculino</div>
        </div>
        """, unsafe_allow_html=True)
